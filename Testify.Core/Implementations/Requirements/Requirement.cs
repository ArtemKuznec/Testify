    ...
    public sealed class Requirement<TComponent, TValue> : Requirement<TComponent> where TComponent : IWebComponent
    {
        ...
        public override bool Execute(TComponent component, TimeSpan? timeout = null)
        {
            try
            {
                ...
                {
                    throw new RequirementException("An exception occurred while retrieving the actual value.", component, exception);
                }

                try { result = _comparer.Invoke(actual, Expected); }
                catch (Exception exception)
                {
                    throw new RequirementException("An exception occurred while comparing the actual and expected value.", component, exception);
                }
            ...
            {
                throw new RequirementException($"An exception occurred while executing the \"{this}\" requirement.", component, exception);
            }
        }
